...
class CLIP(nn.Module):
    ...
    # modules/module_clip.py (CLIP class 안)
    def encode_audio(self, audio):
        # AST는 CLIP dtype(대개 fp16)으로 실행
        a = self.audio(audio.to(self.dtype))     # [B, Ta, 768], fp16

        # 이후 계산은 FP32로 승격
        a = a.to(torch.float32)
        a = self.audio.v.norm(a)                 # LayerNorm을 FP32로

        # 프로젝션은 반드시 FP32 파라미터로
        x = a @ self.audio_projection.float()    # [B, Ta, embed_dim], FP32

        # FP32로 돌려주는 게 이후 모듈(Transformer_Text/Gate가 FP32)과 잘 맞습니다
        return x
